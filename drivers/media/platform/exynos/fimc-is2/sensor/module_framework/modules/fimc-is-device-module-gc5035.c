		...
		struct exynos_platform_fimc_is_module *pdata)
{
	...
	if (use_2nd_module) {
		SET_PIN(pdata, SENSOR_SCENARIO_NORMAL, GPIO_SCENARIO_ON, gpio_none,   SENSOR_SET_DELAY,  PIN_NONE,       0, 500);
	}
	...
	/* READ_ROM - POWER ON */
	SET_PIN(pdata, SENSOR_SCENARIO_READ_ROM, GPIO_SCENARIO_ON, iovdd_pin.gpio, iovdd_pin.name, iovdd_pin.type, 1, 0);
	/* READ_ROM - POWER OFF */
