...
int sensor_gc5035_cis_stream_on(struct v4l2_subdev *subdev)
{
	...
	if (single_mode) {
		info("[gc5035] start with single mode\n");
		/* Delay for single mode */
		msleep(50);
	} else {
		info("[gc5035] start with dual slave mode\n");
...
int sensor_gc5035_cis_set_exposure_time(struct v4l2_subdev *subdev, u16 multiple_exp)
{
	int ret = 0;
	int hold = 0;
	struct fimc_is_cis *cis;
...
int sensor_gc5035_cis_set_analog_gain(struct v4l2_subdev *subdev, u32 input_again)
{
	int ret = 0;
	int hold = 0;
	struct fimc_is_cis *cis;
	...
	analog_gain = sensor_gc5035_cis_calc_again_code(input_again);
	analog_permile = sensor_gc5035_cis_calc_again_permile(analog_gain);
	digital_gain = sensor_gc5035_cis_calc_dgain_code(input_again, analog_permile);
	...
	struct ae_param *again, struct ae_param *dgain)
{
	...
	origin_exp = ((target_exposure->long_val * vt_pic_clk_freq_mhz) - min_fine_int) / line_length_pck;
	...
	dbg_sensor(2, "[MOD:D:%d] %s, frame_length_lines(%#x), multiple_exp %#x\n",
		cis->id, __func__, cis_data->frame_length_lines, multiple_exp);

	/* Set Exposure Time */
	ret = sensor_gc5035_cis_set_exposure_time(subdev, multiple_exp);
	if (ret < 0) {
		err("[%s] sensor_gc5035_cis_set_analog_gain fail\n", __func__);
		goto p_err;
	}

	/* Set Analog & Digital gains */
	input_again = ((float)(origin_exp)/(float)(multiple_exp))*(again->val);
	ret = sensor_gc5035_cis_set_analog_gain(subdev, input_again);
	if (ret < 0) {
		err("[%s] sensor_gc5035_cis_set_analog_gain fail\n", __func__);
		goto p_err;
	}
